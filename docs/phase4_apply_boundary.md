...
Phase 4 may add an explicit apply flow, but it must not weaken the read-only MVP and Phase 3 boundary.
...
Before implementation, Issue #22 must settle these design points:
...
- Missing, malformed, `pending`, or `rejected` review files are rejected before any API call.
...
- The command writes an audit log locally under an ignored output directory.
- Tests must prove that non-approved review states cannot reach the write client.

## Suggested Command Shape

The exact CLI is still open, but the safe shape is:
...
1. #22: design and implement approved-only apply with dry-run first.
